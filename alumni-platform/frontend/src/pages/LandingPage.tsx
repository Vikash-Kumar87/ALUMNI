...
const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0d0b1e] overflow-x-hidden">
      {/* ── Navbar ── */}
      <nav className={`sticky top-0 z-50 transition-all duration-500 relative ${scrolled ? 'bg-indigo-950/95 backdrop-blur-2xl shadow-md border-b border-white/10' : 'bg-gradient-to-r from-indigo-950 to-violet-950 backdrop-blur-md border-b border-white/10'}`}>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-extrabold text-lg tracking-tight hidden min-[380px]:block text-white">AlumniConnect</span>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-1">
            {[
              ...
              <a key={label} href={href}
                className="relative px-4 py-2 text-sm font-semibold text-white/70 hover:text-white hover:bg-white/10 rounded-xl transition-all duration-200 group">
                {label}
          ...
          <div className="flex items-center gap-2">
            {currentUser ? (
              ...
              <>
                <Link to="/login" className="hidden sm:block text-sm font-semibold px-4 py-2 rounded-xl transition-all duration-200 text-white hover:text-white hover:bg-white/10 border border-white/20 hover:border-white/30">
                  Log in
                </Link>
      ...
      <section className="relative overflow-hidden bg-gradient-hero pt-16 pb-24 lg:pt-24 lg:pb-32">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -top-32 -right-32 w-[600px] h-[600px] rounded-full opacity-40 blur-3xl bg-gradient-to-br from-violet-600 to-purple-800 aurora-blob" />
          <div className="absolute -bottom-20 -left-20 w-[400px] h-[400px] rounded-full opacity-30 blur-3xl bg-gradient-to-br from-indigo-700 to-violet-900 aurora-blob-2" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full opacity-10 blur-3xl bg-gradient-to-br from-primary-300 to-violet-400 animate-breathe" />
          <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.15) 1px,transparent 1px),linear-gradient(to right,rgba(255,255,255,0.15) 1px,transparent 1px)', backgroundSize: '64px 64px' }} />
        </div>
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 text-white/90 px-4 py-1.5 rounded-full text-sm font-semibold mb-8 shadow-sm animate-fade-in-down backdrop-blur-sm">
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white leading-[1.1] mb-6 animate-fade-in-up">
            Bridge the Gap Between{' '}
            <span className="text-gradient">Alumni</span>
            {' & '}
            <span className="inline-block bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">Students</span>
          </h1>
          <p className="text-lg sm:text-xl text-white/60 max-w-2xl mx-auto mb-10 leading-relaxed animate-fade-in delay-100">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-white/40 animate-fade-in delay-300">
            {['No credit card required', 'Free to use', 'Secure & private'].map(t => (
              <span key={t} className="flex items-center gap-1.5">
                <FiCheckCircle className="w-4 h-4 text-emerald-400" /> {t}
              </span>
            ))}
          </div>
        </div>
      ...
      {/* Stats */}
      <section className="py-16 bg-[#0f0d24] border-y border-white/5">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 text-center">
          {STATS.map((s, i) => (
            <div key={s.label} className="group animate-on-scroll" style={{ transitionDelay: `${i * 100}ms` }}>
              <div className="w-13 h-13 mx-auto mb-3 rounded-2xl bg-white/5 flex items-center justify-center group-hover:scale-110 group-hover:shadow-soft transition-all duration-300">
      ...
      {/* Features */}
      <section id="features" className="py-24 bg-[#0d0b1e]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16 animate-on-scroll">
            <span className="inline-block px-4 py-1.5 bg-indigo-500/10 border border-indigo-500/20 rounded-full text-indigo-400 text-sm font-semibold mb-4">Features</span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white mb-4 tracking-tight">Everything You Need to Succeed</h2>
            <p className="text-white/50 text-lg max-w-xl mx-auto">One platform, all the tools that matter for your career growth</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((f, i) => (
              <div key={i} className="group cursor-default animate-on-scroll hover:scale-105 hover:-translate-y-2 transition-all duration-400 rounded-2xl p-6 bg-white/5 border border-white/10 backdrop-blur-sm hover:bg-white/8 hover:border-violet-500/30 hover:shadow-[0_0_40px_rgba(139,92,246,0.12)]" style={{ transitionDelay: `${i * 100}ms` }}>
                <div className={`icon-box w-12 h-12 bg-gradient-to-br ${f.gradient} mb-5 shadow-md`}>
                  <f.icon className="w-5 h-5 text-white" />
                </div>
                <h3 className="font-bold text-white mb-2.5 group-hover:text-violet-300 transition-colors">{f.title}</h3>
      ...
      {/* How it works */}
      <section id="how-it-works" className="py-24 bg-[#0f0d24]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16 animate-on-scroll">
            <span className="inline-block px-4 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-emerald-400 text-sm font-semibold mb-4">Process</span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white mb-4 tracking-tight">Get Started in 3 Simple Steps</h2>
            <p className="text-white/50 text-lg">Simple, fast, and designed for your growth</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            <div className="hidden md:block absolute top-8 left-[calc(33%-4px)] right-[calc(33%-4px)] h-0.5 bg-gradient-to-r from-indigo-500/30 via-violet-500/30 to-amber-500/30" />
      ...
      {/* Testimonials */}
      <section id="testimonials" className="py-24 bg-[#0d0b1e]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16 animate-on-scroll">
            <span className="inline-block px-4 py-1.5 bg-violet-500/10 border border-violet-500/20 rounded-full text-violet-400 text-sm font-semibold mb-4">Reviews</span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white mb-4 tracking-tight">Loved by Students & Alumni</h2>
            <div className="flex justify-center gap-1 mt-2">
              {[...Array(5)].map((_, i) => <FiStar key={i} className="w-5 h-5 text-amber-400 fill-amber-400" />)}
              <span className="ml-2 text-sm text-white/50 font-medium">4.9/5 from 200+ reviews</span>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="group animate-on-scroll hover:scale-105 hover:-translate-y-2 transition-all duration-400 rounded-2xl p-6 bg-white/5 border border-white/10 hover:border-violet-500/30 hover:shadow-[0_0_40px_rgba(139,92,246,0.12)]" style={{ transitionDelay: `${i * 120}ms` }}>
